...
A[User Clicks Login] --> B[Redirect to Salesforce Auth URL]
B --> C[User Login & Consent]
C --> D[Authorization Code Returned]

D --> E[Backend Sends Code + Client Secret]
E --> F[Salesforce Token Endpoint]

F --> G[Access Token + Refresh Token]

G --> H[Call Salesforce API]
H --> I[Data Returned]

G --> J[Token Expired]
J --> K[Use Refresh Token]
K --> G
```
